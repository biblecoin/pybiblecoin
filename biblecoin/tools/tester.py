from biblecoin.utils import sha3, privtoaddr, int_to_addr, to_string, big_endian_to_int, checksum_encode, int_to_big_endian, encode_hex, normalize_address
from biblecoin.genesis_helpers import mk_basic_state
...
from biblecoin.tools._solidity import get_solidity
...
class Chain(object):
    def __init__(self, alloc=base_alloc, env=None, genesis=None):
        from biblecoin.pow import chain as pow_chain
        if genesis:
            if type(genesis)!=dict and genesis.env.config['CONSENSUS_STRATEGY'] == 'hybrid_casper':
                from biblecoin.hybrid_casper import chain as hybrid_casper_chain
